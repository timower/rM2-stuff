...
| `+0x55` | `uint8 fastDraw` | The `FastDraw` update flag (`update_data.flags` bit 1) - named `fullRefresh` until this was corrected; see `tools/qsgepaper-preload/swtcon.h`'s `UpdateFlags` comment for the evidence trail (queue_update itself never gives this bit a semantic clue; the real signal is in `swapBuffers_impl`, which sets it for pen/async fast-UI draws and never for the two-pass synchronous full-screen clear). |
| `+0x56` | — | padding[2] |
