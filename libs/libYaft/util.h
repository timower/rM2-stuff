...
ewrite(int fd, const void* buf, size_t size) {
  ...
  if ((ret = write(fd, buf, size)) < 0) {
    if (errno == EINTR) {
      ...
      logging(ERROR,
              "write: EAGAIN or EWOULDBLOCK occurred, sleep %d usec\n",
              SLEEP_TIME);
      usleep(SLEEP_TIME);
      return ewrite(fd, buf, size);
    } else {
...
eopenpty(int* amaster,
  ...
  if ((master = posix_openpt(O_RDWR | O_NOCTTY)) < 0 || grantpt(master) < 0 ||
      unlockpt(master) < 0 || (name = ptsname(master)) == NULL) {
...
eforkpty(int* amaster,
  ...
  if (pid < 0) {
    ...
    /* XXX: this ioctl may fail in Mac OS X
            ref
       http://www.opensource.apple.com/source/Libc/Libc-825.25/util/pty.c?txt */
