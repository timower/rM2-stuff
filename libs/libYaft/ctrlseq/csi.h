...
void
insert_blank(struct terminal_t* term, struct parm_t* parm) {
  int i, num = sum(parm);
...
void
curs_up(struct terminal_t* term, struct parm_t* parm) {
  int num = sum(parm);
...
void
curs_down(struct terminal_t* term, struct parm_t* parm) {
  int num = sum(parm);
...
void
curs_forward(struct terminal_t* term, struct parm_t* parm) {
  int num = sum(parm);
...
void
curs_back(struct terminal_t* term, struct parm_t* parm) {
  int num = sum(parm);
...
void
curs_nl(struct terminal_t* term, struct parm_t* parm) {
  int num = sum(parm);
...
void
curs_pl(struct terminal_t* term, struct parm_t* parm) {
  int num = sum(parm);
...
void
curs_col(struct terminal_t* term, struct parm_t* parm) {
  int num;
...
void
curs_pos(struct terminal_t* term, struct parm_t* parm) {
  int line, col;
...
void
curs_line(struct terminal_t* term, struct parm_t* parm) {
  int num;
...
void
erase_display(struct terminal_t* term, struct parm_t* parm) {
  int i, j, mode;
...
void
erase_line(struct terminal_t* term, struct parm_t* parm) {
  int i, mode;
...
void
insert_line(struct terminal_t* term, struct parm_t* parm) {
  int num = sum(parm);
...
void
delete_line(struct terminal_t* term, struct parm_t* parm) {
  int num = sum(parm);
...
void
delete_char(struct terminal_t* term, struct parm_t* parm) {
  int i, num = sum(parm);
...
void
erase_char(struct terminal_t* term, struct parm_t* parm) {
  int i, num = sum(parm);
...
uint8_t
rgb2index(uint8_t r, uint8_t g, uint8_t b) {
    ...
    */
    ...
    /*
            search from 6x6x6 color cube
                    index: 16 - 231
                    each rgb takes 6 values: {0x00, 0x5F, 0x87, 0xAF, 0xD7,
       0xFF}
    */
    ...
    index =
      16 + closest_index[0] * 36 + closest_index[1] * 6 + closest_index[2];
  }
...
void
set_attr(struct terminal_t* term, struct parm_t* parm) {
   ...
   *		38: special foreground color selection: implemented in
   *select_color_value()
   *
   *		40-47: select background color (color index 0-7)
   *		48: special foreground color selection: implemented in
   *select_color_value()
   */
  int i, num;
  ...
  for (i = 0; i < parm->argc; i++) {
    ...
    if (num == 0) { /* reset all attribute and color */
      ...
      term->attribute |= (enum char_attr)attr_mask[num];
    } else if (21 <= num && num <= 27) { /* reset attribute */
      term->attribute &= (enum char_attr) ~attr_mask[num - 20];
...
void
status_report(struct terminal_t* term, struct parm_t* parm) {
  int i, num;
  char buf[BUFSIZE];
  ...
  for (i = 0; i < parm->argc; i++) {
    ...
    if (num == 5) { /* terminal response: ready */
      ...
      snprintf(
        buf, BUFSIZE, "\033[%d;%dR", term->cursor.y + 1, term->cursor.x + 1);
...
void
device_attribute(struct terminal_t* term, struct parm_t* parm) {
  /* TODO: refer VT525 DA */
  (void)parm;
...
void
set_mode(struct terminal_t* term, struct parm_t* parm) {
  int i, mode;
...
void
reset_mode(struct terminal_t* term, struct parm_t* parm) {
  int i, mode;
...
void
set_margin(struct terminal_t* term, struct parm_t* parm) {
  int top, bottom;
  ...
  if (parm->argc <= 0) { /* CSI r */
    ...
    bottom =
      (parm->argv[1] == NULL) ? term->lines - 1 : dec2num(parm->argv[1]) - 1;
  } else {
    return;
  }
...
void
clear_tabstop(struct terminal_t* term, struct parm_t* parm) {
  int i, j, num;
