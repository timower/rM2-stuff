...
constexpr size_t kGammaTableSize =
  0x4400; // == 128 temperature entries * 0x88 bytes
constexpr size_t kLutBlobSize =
  0x165800; // one full frame slot's worth (bitsPerPixel*xres*yres/8, see
            // FbInitParams in swtcon.cpp)

// Size of the whole panel framebuffer (all kFrameSlotRingCount+1 frame
// slots) - what swtcon_init's real /dev/fb0 mmap covers.
constexpr size_t kFramebufferMemorySize =
  (size_t)(kFrameSlotRingCount + 1) * kLutBlobSize;
...
static_assert(sizeof(FbInitParams) == 14 * sizeof(int32_t),
              "FbInitParams must match the library's 14-int parameter block");
...
int
init_statebuffer(void* dataBuffer = nullptr, void* backBuffer = nullptr);
int
