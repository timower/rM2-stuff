...
void (*ctrl_func[CTRL_CHARS])(struct terminal_t* term) = {
  [BS] = bs, [HT] = tab, [LF] = nl,         [VT] = nl,
  [FF] = nl, [CR] = cr,  [ESC] = enter_esc,
};

void (*esc_func[ESC_CHARS])(struct terminal_t* term) = {
  ['7'] = save_state, ['8'] = restore_state, ['D'] = nl,
  ['E'] = crnl,       ['H'] = set_tabstop,   ['M'] = reverse_nl,
  ['P'] = enter_dcs,  ['Z'] = identify,      ['['] = enter_csi,
  [']'] = enter_osc,  ['c'] = ris,
};
...
void
control_character(struct terminal_t* term, uint8_t ch) {
...
void
esc_sequence(struct terminal_t* term, uint8_t ch) {
  *term->esc.bp = '\0';
...
void
csi_sequence(struct terminal_t* term, uint8_t ch) {
  struct parm_t parm;
...
void
omit_string_terminator(char* bp, uint8_t ch) {
...
void
osc_sequence(struct terminal_t* term, uint8_t ch) {
  int osc_mode;
  struct parm_t parm;
...
void
dcs_sequence(struct terminal_t* term, uint8_t ch) {
  char* cp;
...
void
utf8_charset(struct terminal_t* term, uint8_t ch) {
  if (0x80 <= ch && ch <= 0xBF) {
    ...
    if ((term->charset.following_byte == 0) ||
        (term->charset.following_byte == 1 && term->charset.count == 0 &&
         term->charset.code <= 1) ||
        (term->charset.following_byte == 2 && term->charset.count == 0 &&
         term->charset.code == 0 && ch < 0xA0) ||
        (term->charset.following_byte == 3 && term->charset.count == 0 &&
         term->charset.code == 0 && ch < 0x90) ||
        (term->charset.following_byte == 4 && term->charset.count == 0 &&
         term->charset.code == 0 && ch < 0x88) ||
        (term->charset.following_byte == 5 && term->charset.count == 0 &&
         term->charset.code == 0 && ch < 0x84))
  ...
  if (term->charset.count >= term->charset.following_byte) {
    /*	illegal code point (ref:
       ...
       noncharacter (n: 0x00 ~ 0x10) 0x110000 ~        : invalid (unicode U+0000
       ~ U+10FFFF)
    */
    if (!term->charset.is_valid ||
        ...
        ((term->charset.code & 0xFFFF) == 0xFFFE ||
         (term->charset.code & 0xFFFF) == 0xFFFF) ||
...
void
parse(struct terminal_t* term, uint8_t* buf, int size) {
  /*
