...
int put_cmap(int fd, cmap_t *cmap) { return ioctl(fd, FBIOPUTCMAP, cmap); }

int get_cmap(int fd, cmap_t *cmap) { return ioctl(fd, FBIOGETCMAP, cmap); }

/* initialize struct fb_info_t */
void set_bitfield(struct fb_var_screeninfo *vinfo, struct bitfield_t *red,
                  struct bitfield_t *green, struct bitfield_t *blue) {
...
bool set_fbinfo(int fd, struct fb_info_t *info) {
  ...
  /* check screen [xy]offset and initialize because linux console changes these
   * values */
