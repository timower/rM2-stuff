...
myMain(int argc, char** argv, char** env) {
  ...
  TIME(swtcon_update(state, { 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT }, HQ, FullRefresh | Sync));
  ...
  TIME(swtcon_update(state, { 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT }, MEDIUM, FullRefresh | Sync));
  std::cout << "Done" << std::endl;
  getchar();
  ...
  y = 2 * SCREEN_HEIGHT / 6;
  std::cout << "Testing direct update (partial + FastDraw): " << y << std::endl;
  for (int x = 0; x < SCREEN_WIDTH; x++) {
    image[(y - 1) * SCREEN_WIDTH + x] = 0;
    image[y * SCREEN_WIDTH + x] = 0;
    image[(y + 1) * SCREEN_WIDTH + x] = 0;
  }
  TIME(swtcon_update(state, { 0, y - 2, SCREEN_WIDTH, y + 2 }, FAST, FastDraw | Sync));
  std::cout << "Done" << std::endl;
...
__libc_start_main(int (*_main)(int, char**, char**),
  ...
  static auto* func_main = (decltype(&__libc_start_main))dlsym(RTLD_NEXT, "__libc_start_main");
