  ...
  runCommand "rm-${fw_version}.qcow2"
    {
      ...
      passthru.ext4_image = rootfsImage;
    ...
    ''
      ...
      ${lib.optionalString (extraCommands != null) extraCommands}

      umount /mnt/home

      # Disable auto-reboot if xochitl fails.
      ln -s /dev/null /mnt/root/etc/systemd/system/remarkable-fail.service

      # Disable updates, they work, but slow down the system.
