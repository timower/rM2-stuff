...
- **Phase 6 — `render_update_kernel` / `dispatch_update_regions`:** TODO, reversed. Mode dispatch table and per-pixel formulas for all cases are derived (not yet byte-verified); the dispatcher's control flow and output struct are confirmed. The only two library calls left in the `swtcon_update` path itself.
...
1. **A/B-verify the derived formulas** before trusting them enough to ship native code: `render_update_kernel`'s per-pixel bitfield semantics, `FUN_0004a140`'s NEON shift constants, the display-commit kernels' `(state<<5)|value` packing, and the display thread's frame-pacing target formula (precisely reversed, see `swtcon_architecture.md` §6.2 step 5 — but still worth an A/B pass before depending on it). Same technique already used for the gamma/LUT/statebuffer tables — write a native candidate, diff its output byte-for-byte against the library on identical input, let mismatches point at the wrong guess.
2. **Port `dispatch_update_regions`/`render_update_kernel`** (Phase 6) — the only remaining update-path leaves, and now the largest remaining native-porting chunk in the whole project.
