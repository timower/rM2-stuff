...
`swtcon_update`'s entire path is now fully native — zero remaining by-address library calls in it. In the display pipeline (Phase 5), `dispatch_processed_regions` (0x50660) and its two display-commit kernels (`FUN_0004f8f0`/`FUN_0004e680`) are now native and wired in (see above), and so is `FUN_0004a140` (see below); `FUN_0004a234` (the "overlap-aware" worker-side playback kernel) remains genuinely unreversed and by-address — the only remaining by-address call in the whole pipeline.
...
1. **Reverse `FUN_0004a234`** (0x4a234, the "overlap-aware" variant) — the only remaining by-address call in the whole pipeline. A many-KB function with its own 9-way jump table on `frameCount` and three further unreversed delegates (`FUN_0004a3f8`/`FUN_0004a9e0`/`FUN_0004b098`) for cases 1–3. Apply the same two-pronged technique that unblocked `FUN_0004a140`: black-box calls through `playback_kernel_probe.cpp` first, and read the Ghidra decompile directly (`mcp__ghidra__decompile_function` on `libqsgepaper_3.23.0.54.so`) as soon as guessing stalls rather than continuing to iterate blind. It's reached whenever `intList` has no active dependency and `phase` is 8-aligned (see `swtcon_architecture.md` §6.4) — not just the very first HQ update as an earlier pass assumed, and per the `KERN` log it's actually the far more common of the two kernels in the standard test matrix (117 overlap dispatches vs. 2 plain, in one run). `ab_capture_kernel`'s `KERN` log (wired into both `native_dispatch_plain_kernel`/`native_dispatch_overlap_kernel` in `native_display.cpp`) records every real dispatch's `(kernel, frameCount, chunkCount)` so the emulator test matrix's actual coverage of this kernel's cases is known rather than assumed — check that log's coverage before trusting any case as tested. Once `FUN_0004a234` is native, `libqsgepaper.so` no longer needs to be `dlopen`'d at all — this is the last item standing between the project and full independence.
2. **A/B-verify the remaining derived formulas** before trusting them enough to ship native code: the display thread's frame-pacing target formula (precisely reversed, see `swtcon_architecture.md` §6.2 step 5 — but still worth an A/B pass before depending on it). `FUN_0004f8f0`/`FUN_0004e680`'s packing formula no longer needs this — it's confirmed (see above). Same technique as `render_kernel_verify.cpp`/`render_kernel_addr_map.cpp` — write a native candidate, diff its output byte-for-byte (or via an addressing-independent invariant) against the library on identical input.
3. Apply the same Ghidra rename pass already done for other native functions to the remaining confirmed-but-unnamed ones (`FUN_0004f8f0`/`FUN_0004e680`/`FUN_0003ec78`/`FUN_0003f294`/`FUN_0003f1f0`/`FUN_0004a140`); hold off on `FUN_0004a234` and its three delegates until they're actually fully ported.
4. **Hardware re-confirmation** — Phases 4b, 5, and 6 plus the now-wired-in `dispatch_processed_regions`/`FUN_0004a140`'s emulator-only confirmations should be re-run on real hardware when next convenient.
