...
| `+0x55` | `uint8 fastDraw` | The `FastDraw` update flag (`update_data.flags` bit 1) - named `fullRefresh` until this was corrected; see `libs/swtcon/include/swtcon.h`'s `UpdateFlags` comment for the evidence trail (queue_update itself never gives this bit a semantic clue; the real signal is in `swapBuffers_impl`, which sets it for pen/async fast-UI draws and never for the two-pass synchronous full-screen clear). |
| `+0x56` | — | padding[2] |
...
Both fully ported (`dispatch_update_regions`/`render_update_kernel` in `update.cpp`). The subsections below describe the *library's own* algorithm (allocation logic, formulas, addressing) as reversed and confirmed — the native port implements the same formulas/addressing (§5.2) **and now also replicates the library's own two-way thread-pool chunking** (§5.1 point 4, via `update_chunk_count`/two `std::thread`s in `dispatch_update_regions`). That chunking is provably invisible in this function's own *output* (see §5.2's addressing box) — it was originally left unreplicated on that basis — but it turned out not to be invisible overall: `dispatch_update_regions` runs synchronously on the caller's own thread before the item is even queued, so running it single-threaded measurably slows down large-rect updates under emulation (confirmed: ~155ms single-threaded vs ~65ms two-threaded for a 1035-column rect), a real gap from the real library's own performance envelope worth closing on its own terms. Found while investigating `tools/swtcon-test/main.cpp` cases 24-27's A2-priming mismatch, but ruled out as that mismatch's own cause: the frame number a newly-dispatched item lands on is forced to the same value regardless of how long this call takes (confirmed both analytically and empirically). That mismatch's actual cause turned out to be an unrelated bordering outline rect the test itself used to draw around each grid, forcing a ring-frame-slot collision specific to A2's short LUT - see `ab_compare.sh`'s header comment for the full history; removing that unneeded border rect from the test resolved it.
